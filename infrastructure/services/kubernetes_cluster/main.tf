locals {
  wait = length(module.kubernetes_cluster.endpoint
    ...
    ) + length(kubernetes_secret.build_job_gce_service_account_key.id
...
#                       credentialsId: 'build-job-gce-service-account-key',
...
resource "kubernetes_secret" "build_job_gce_service_account_key" {
  depends_on = [ var.module_depends_on ]

  metadata {
    # Credential name is the identifier used in the withCredentials() call
    name = "build-job-gce-service-account-key"
    labels = {
  ...
  data = {
    ...
    "filename" = "build-job-gce-service-account-key.json"
  }
}
