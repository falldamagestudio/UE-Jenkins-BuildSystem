...
data "http" "linux_ssh_agent_vm_cloud_config" {
  count = (var.docker_ssh_agent.linux.vm_cloud_config_url != "" ? 1 : 0)
  url = var.docker_ssh_agent.linux.vm_cloud_config_url
...
resource "google_compute_instance_template" "linux_agent_template" {
    ...
    for_each = var.docker_dynamic_agent_templates.linux
    ...
    disk {
        source_image = var.docker_ssh_agent.linux.vm_image_name
    ...
    metadata = {
        ...
        user-data = data.http.linux_ssh_agent_vm_cloud_config[0].body
    }
}

resource "google_compute_instance_template" "windows_agent_template" {

    for_each = var.docker_dynamic_agent_templates.windows
    ...
    disk {
        source_image = var.docker_ssh_agent.windows.vm_image_name
