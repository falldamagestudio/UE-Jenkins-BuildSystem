...
helm upgrade --install --values "${APPLICATION_DIR}/values/values.yaml" --values "${APPLICATION_DIR}/values/local/jenkinsUrl.yaml" --values "${APPLICATION_DIR}/values/local/local-image.yaml" --version=${CHART_VERSION} ${RELEASE} ${CHART} --debug --set controller.image=${CONTROLLER_IMAGE} --set controller.tag=${CONTROLLER_IMAGE_TAG} --set GOOGLE_OAUTH_CLIENT_ID=${GOOGLE_OAUTH_CLIENT_ID} --set GOOGLE_OAUTH_CLIENT_SECRET=${GOOGLE_OAUTH_CLIENT_SECRET}  --set controller.containerEnv[0].name=UE_JENKINS_BUILDTOOLS_IMAGE --set controller.containerEnv[0].value=${UE_JENKINS_BUILDTOOLS_IMAGE} --set controller.containerEnv[1].name=UE_JENKINS_BUILDTOOLS_IMAGE_TAG --set controller.containerEnv[1].value=${UE_JENKINS_BUILDTOOLS_IMAGE_TAG} --wait
